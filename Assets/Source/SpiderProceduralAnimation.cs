...
public class SpiderProceduralAnimation : MonoBehaviour
{
    ...
    Vector3[] MatchToSurfaceFromAbove(Vector3 point, float halfRange, Vector3 up)
    {
        ...
        if (Physics.SphereCast(ray, sphereCastRadius, out hit, 2f * halfRange, GroundLayer))
        {
            res[0] = hit.point;
            res[1] = hit.normal;
        }
    ...
    void Start()
    {
        ...
        {
            defaultLegPositions[i] = transform.InverseTransformPoint(legTargets[i].position);
